...
{
	struct st7701 *ctx = panel_to_st7701(panel);
	int ret;
...
static int st7701_enable(struct drm_panel *panel)
{
	...
	ST7701_DSI(ctx, MIPI_DCS_SET_DISPLAY_ON, 0x00);
...
static int st7701_disable(struct drm_panel *panel)
{
	...
	ST7701_DSI(ctx, MIPI_DCS_SET_DISPLAY_OFF, 0x00);
...
static int st7701_unprepare(struct drm_panel *panel)
{
	...
	ST7701_DSI(ctx, MIPI_DCS_ENTER_SLEEP_MODE, 0x00);
...
static int st7701_dsi_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx->supply = devm_regulator_get(&dsi->dev, "supply");
	 ...
	 */
	ctx->sleep_delay = 120 + desc->panel_sleep_delay;
...
{
	struct st7701 *ctx = mipi_dsi_get_drvdata(dsi);
